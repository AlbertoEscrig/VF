...
// ---------------------------------------------------------------------------------- Momento lineal
  ...
  TSistema const UEc =
    div(ρ * U * U) - μ * lap(U) == μ / 3.0 * lapT(U) - grad(p) - (g & TMalla2D::C()) * grad(ρ);

  solve(UEc, U, α);

// ------------------------------------------------------------------------------------- Continuidad

  // CC Neumann homogéneas (https://doc.cfd.direct/notes/cfd-general-principles/boundary-fluxes)
  TCampo const φ = ρ * (UEc.b + grad(p) - UEc.ΣaN(U)) / UEc.aP;
