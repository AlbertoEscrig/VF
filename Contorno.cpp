...
public:
  ...
  std::tuple<double, TTensor<d, r>>
  virtual GradCoef(TCara<d> const &Cara) const override
    { return {-h / (k + h * mag(Cara.L())), (1.0 / (k + h * mag(Cara.L()))) * hφb}; }
};
...
public:
  ...
  TPeriodica(TCampo<d, r> const &φ_, TTensor<d, 2u> const &T_ = TTensor<d, 2u>::Identidad()) :
    φ(φ_), T(T_) {}
...
TTensor<d, 2u> const T = TTensor<d, 2u>::Identidad() - 2.0 * Sf * Sf / (Sf & Sf);
